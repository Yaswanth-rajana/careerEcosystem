...
export default function MentorProfilePage() {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col justify-between bg-[#FAFAFC] text-slate-900">
        <Header />
    ...
    return (
      <div className="min-h-screen flex flex-col justify-between bg-[#FAFAFC] text-slate-900">
        ...
        <main className="flex-1 max-w-xl mx-auto px-4 py-20 w-full text-center space-y-4">
          ...
          <h1 className="text-2xl font-bold font-display text-slate-900">Mentor Profile Not Found</h1>
          <p className="text-sm text-slate-600">
            The mentor profile you are looking for may have been updated or is no longer available.
          </p>
          <div className="pt-4">
            <Link href="/mentors">
              <Button variant="primary" leftIcon={<ArrowLeft className="w-4 h-4" />} className="bg-blue-600 hover:bg-blue-700 text-white">
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 text-left">
            <div className="p-6 sm:p-8 rounded-3xl border border-slate-200/80 bg-white shadow-sm space-y-8">
              
              {/* Header Info */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
                <div className="relative w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden border-2 border-slate-200 shrink-0 shadow-md">
                ...
                <div className="space-y-2 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-blue-50 text-blue-600 text-xs font-bold">
                      <ShieldCheck className="w-3.5 h-3.5" />
                      <span>{mentor.badgeLabel}</span>
                    </div>
                  </div>

                  <p className="text-base font-semibold text-slate-700">{mentor.role}</p>
              ...
              <div className="pt-6 border-t border-slate-100 space-y-3">
                <h2 className="text-xl font-bold font-display text-slate-900">About</h2>
              ...
              <div className="pt-6 border-t border-slate-100 space-y-4">
                <h2 className="text-xl font-bold font-display text-slate-900">Expertise & Skills</h2>
                
                <div>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 font-display">
                    Domains
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {mentor.expertise.map((exp) => (
                      <span key={exp} className="px-3 py-1 rounded-xl bg-blue-50 text-blue-600 text-xs font-bold">
                        {exp}
                      </span>
                    ))}
                  </div>
                ...
                <div className="pt-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {mentor.skillsList.map((skill) => (
                      <span key={skill} className="px-3 py-1 rounded-xl bg-slate-100 text-slate-700 text-xs font-medium">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              <div className="pt-6 border-t border-slate-100 space-y-4">
                <h2 className="text-xl font-bold font-display text-slate-900">I Can Help You With</h2>
              ...
              {/* Experience Timeline */}
              <div className="pt-6 border-t border-slate-100 space-y-4">
                <h2 className="text-xl font-bold font-display text-slate-900">Professional Experience</h2>
                <div className="space-y-4">
                  {mentor.workHistory.map((work, i) => (
                    <div key={i} className="flex items-start gap-3.5">
                      ...
                      <div>
                        <h4 className="font-bold text-sm text-slate-900 font-display">{work.role}</h4>
                        <p className="text-xs text-slate-500">{work.organizationType} · {work.period}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="lg:col-span-4 sticky top-24 space-y-6 text-left">
            <div className="p-6 rounded-3xl border border-slate-200/80 bg-white shadow-lg space-y-6">
              <div>
                <h3 className="text-lg font-bold font-display text-slate-900">Session Options</h3>
                <p className="text-xs text-slate-500 mt-1">Book 1-on-1 private guidance</p>
              </div>
              ...
              <div className="space-y-4">
                {mentor.sessionOptions?.map((opt: MentorSessionOption) => (
                  <div
                    key={opt.id}
                    className="p-4 rounded-2xl border border-slate-200 bg-slate-50/50 space-y-3 hover:border-blue-500/60 transition-colors"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <h4 className="font-bold text-xs text-slate-900 font-display">{opt.title}</h4>
                    ...
                    <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1">
                      ...
                      <Button
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold py-1.5 px-3"
                      >
                        Book Session
                      </Button>
                    </div>
              ...
              {/* Availability Note */}
              <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center gap-2.5 text-xs text-emerald-700 font-semibold">
