...
export const AuthShell: React.FC<AuthShellProps> = ({ mode, onSwitchMode }) => {
  ...
  return (
    <div className="w-full max-w-[1160px] min-h-[650px] mx-auto rounded-[24px] sm:rounded-[32px] border border-slate-200 bg-white shadow-xl overflow-hidden relative">
      ...
      <div className="hidden lg:grid lg:grid-cols-2 relative w-full min-h-[650px] overflow-hidden">
        ...
        <motion.div
          ...
          className="w-full h-full flex items-center justify-center p-10 xl:p-14 z-20 bg-white"
        >
      ...
      <div className="lg:hidden flex flex-col w-full min-h-[650px] p-4 sm:p-6 space-y-6">
        ...
        {/* Bottom Form Panel */}
        <div className="w-full flex-1 flex items-center justify-center p-4 sm:p-8 bg-white">
          {mode === 'signin' ? (
            <SignInForm onSwitchMode={handleToggle} />
          ) : (
            <SignUpForm onSwitchMode={handleToggle} />
