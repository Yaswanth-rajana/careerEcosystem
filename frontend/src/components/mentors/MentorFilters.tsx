  ...
  return (
    <div className="relative inline-block text-left shrink-0" ref={dropdownRef}>
      ...
      <button
        ...
        className={`flex items-center gap-2 px-4 py-3 rounded-xl border text-sm font-semibold transition-all duration-200 shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-indigo ${
          isOpen || activeCount > 0
            ? 'border-brand-indigo bg-brand-indigo/5 dark:bg-brand-indigo/10 text-brand-indigo dark:text-brand-indigo-light'
            : 'border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/60'
        }`}
      ...
      >
        ...
        <span>Filters</span>
        {activeCount > 0 && (
          <span className="px-2 py-0.5 rounded-full bg-brand-indigo text-white text-xs font-bold leading-none">
            {activeCount}
          </span>
        )}
        <ChevronDown
      ...
      {isOpen && (
        <div className="absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] sm:w-[560px] md:w-[640px] max-w-[92vw] rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] shadow-2xl z-50 p-5 overflow-hidden animate-in fade-in zoom-in-95 duration-150">
          {/* Header & Reset */}
          <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-slate-800">
            ...
            <button
              ...
              className="text-xs font-semibold text-brand-indigo dark:text-brand-indigo-light hover:underline flex items-center gap-1"
            >
          ...
          <div className="max-h-[70vh] sm:max-h-[75vh] overflow-y-auto overflow-x-hidden pr-2 grid grid-cols-1 sm:grid-cols-2 gap-6 text-left [scrollbar-gutter:stable]">
            ...
            <div className="space-y-5">
              {/* EXPERTISE */}
              <div>
                <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2 font-display">
                  Expertise
                </h4>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-colors text-left ${
                          isChecked
                            ? 'bg-brand-indigo/10 text-brand-indigo dark:text-brand-indigo-light font-bold'
                            : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/60'
                        }`}
                      >
                        <span>{item}</span>
                        {isChecked && <Check className="w-3.5 h-3.5 text-brand-indigo" />}
                      </button>
                    );
                  })}
                </div>
                  ...
                  {SESSION_TYPE_OPTIONS.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-colors text-left ${
                          isChecked
                            ? 'bg-brand-violet/10 text-brand-violet dark:text-brand-violet-light font-bold'
                            : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800/60'
                        }`}
                      >
                        <span>{item}</span>
                        {isChecked && <Check className="w-3.5 h-3.5 text-brand-violet" />}
                      </button>
                    );
                  })}
                </div>
          ...
          <div className="pt-3 mt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end">
            <Button
              variant="primary"
              size="sm"
              onClick={() => setIsOpen(false)}
              className="bg-brand-gradient text-white text-xs font-bold px-4 py-2"
            >
              Done
            </Button>
          </div>
