  ...
  return (
    ...
    >
      <div className="space-y-6 text-left">
        {/* Mentor Mini Header */}
        <div className="flex items-center gap-3.5 p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
          <div className="relative w-12 h-12 rounded-full overflow-hidden border border-slate-300 dark:border-slate-700 shrink-0">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">{mentor.role} · {mentor.domain}</p>
          </div>
        </div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2 font-display">
                Select Session Type
              </label>
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-xl border cursor-pointer transition-all ${
                        isSelected
                          ? 'border-brand-indigo bg-brand-indigo/5 dark:bg-brand-indigo/10 shadow-xs'
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 text-[11px] font-semibold text-slate-500">
                        <span className="flex items-center gap-1">
                          <Clock className="w-3 h-3 text-brand-violet" /> {opt.durationMin} min
                        </span>
                        <span>·</span>
                        <span className="text-brand-indigo">{opt.type}</span>
                      </div>
                    </div>
                  );
                })}
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl border text-xs font-semibold text-left transition-colors flex items-center justify-between ${
                        isSelected
                          ...
                          : 'border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60'
                      }`}
                    >
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <Button
                ...
                className="bg-brand-gradient text-white font-bold text-xs"
              >
                Continue to Review
              </Button>
            </div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1.5 font-display">
              ...
              <textarea
                ...
                rows={3}
                className="w-full p-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#111827] text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-indigo"
              />
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-2 text-xs">
              <div className="flex justify-between text-slate-500">
                ...
                <span className="font-bold text-slate-900 dark:text-white">{selectedOption.title}</span>
              </div>
              <div className="flex justify-between text-slate-500">
                <span>Duration:</span>
                <span className="font-semibold text-slate-900 dark:text-white">{selectedOption.durationMin} mins</span>
              </div>
              <div className="flex justify-between text-slate-500">
                <span>Preferred Slot:</span>
                <span className="font-semibold text-emerald-600 dark:text-emerald-400">{selectedSlot}</span>
              </div>
              <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between font-bold text-sm">
                <span>Estimated Total:</span>
                <span className="text-brand-indigo dark:text-brand-indigo-light">₹{selectedOption.price.toLocaleString()}</span>
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-5 text-center py-2">
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-left text-xs space-y-1">
              <p className="font-bold text-slate-900 dark:text-white">Request Context Preview:</p>
            ...
            <div className="pt-4 border-t border-slate-200 dark:border-slate-800">
              <Button variant="primary" size="md" className="w-full bg-brand-indigo text-white font-bold text-xs" onClick={handleClose}>
