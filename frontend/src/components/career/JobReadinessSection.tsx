...
export const JobReadinessSection: React.FC<JobReadinessSectionProps> = ({ career, candidate }) => {
  ...
  return (
    <div id="job-readiness" className="p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-white via-indigo-50/40 to-slate-50 dark:from-[#111827] dark:via-slate-900/60 dark:to-[#0B0F19] border border-indigo-200/80 dark:border-slate-800 shadow-md text-left space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2 max-w-xl">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-100 dark:bg-indigo-950/80 text-[#6366F1] dark:text-[#818CF8] text-xs font-bold">
        ...
        <div className="flex flex-col items-start md:items-end gap-3 shrink-0">
          <Link
            href="#matching-jobs"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] text-white text-xs font-bold shadow-md hover:opacity-95 transition-opacity"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 pt-2">
        {bars.map((b) => (
          <div key={b.label} className="p-3 rounded-xl bg-white/80 dark:bg-slate-900/60 border border-slate-200/60 dark:border-slate-800 space-y-1.5 text-xs">
            <div className="flex items-center justify-between">
              <span className="font-semibold text-slate-700 dark:text-slate-300">{b.label}</span>
              <span className="font-mono font-bold text-slate-900 dark:text-white">{b.percent}%</span>
            </div>
