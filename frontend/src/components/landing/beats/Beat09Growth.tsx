    ...
    >
      <div className="max-w-6xl mx-auto text-left space-y-6">
        ...
        <div className="text-xs font-semibold uppercase tracking-wider text-slate-500 flex items-center gap-2">
          <Repeat className="w-3.5 h-3.5 text-blue-600 animate-spin" style={{ animationDuration: '12s' }} />
                  ...
                  >
                    {isActive && !shouldReduceMotion && (
                      <motion.div
                        ...
                        className="absolute -inset-1.5 rounded-full border-2 border-blue-600 pointer-events-none"
                      />
                    )}
                    ...
                    <div
                      className={`w-11 h-11 sm:w-12 sm:h-12 rounded-full border-2 flex items-center justify-center transition-all duration-300 shadow-md ${
                        isActive
                          ...
                          : 'bg-white border-slate-300 text-blue-600 hover:border-blue-500 group-hover:scale-105 shadow-sm'
                      }`}
                    >
                      <IconComponent className={`w-5 h-5 transition-colors ${isActive ? 'text-white' : 'text-blue-600'}`} />
                    </div>
                  </motion.div>
                </div>
                  ...
                  <motion.div
                    ...
                    className={`w-44 lg:w-48 p-3.5 rounded-2xl backdrop-blur-md transition-all duration-200 border text-center cursor-pointer ${
                      isActive
                        ...
                        : 'bg-white/80 border-slate-200 hover:border-blue-300 shadow-sm'
                    }`}
                  >
                    <span className="text-[10px] font-extrabold font-mono tracking-wider text-blue-600 block mb-0.5">
                      0{idx + 1}
                    </span>
                    <h4 className="text-xs font-bold font-display uppercase tracking-wider text-slate-900 mb-0.5">
                      {node.title}
                    </h4>
                ...
                >
                  <div
                    className={`relative z-10 w-11 h-11 rounded-full border-2 flex items-center justify-center shrink-0 transition-all duration-300 ${
                      isActive
                        ? 'bg-blue-600 text-white border-white shadow-md scale-105'
                        : 'bg-white border-slate-300 text-blue-600'
                    }`}
                  >
                    <IconComponent className={`w-4 h-4 ${isActive ? 'text-white' : 'text-blue-600'}`} />
                  </div>
                  ...
                  >
                    ...
                    <h3 className="text-xs font-bold font-display uppercase tracking-wider text-slate-900 mb-0.5">
                      {node.title}
                    </h3>
